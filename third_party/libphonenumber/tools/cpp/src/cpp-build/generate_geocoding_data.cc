...
// success.
bool ListDirectory(const boost::filesystem::path& path, vector<DirEntry>* entries) {
  entries->clear();
  ...
  for (; it != end_it; ++it ) {
    if ((it->path().leaf().string() == ".") || (it->path().leaf().string() == "..")) continue;
    ...
    entries->push_back(DirEntry(it->path().leaf().string().c_str(), kind));
  }
  return true;
}
...
bool WriteSource(const string& data_path, const string& base_name,
  ...
  boost::filesystem::path datapath = data_path;
  if (!ListDirectory(datapath, &entries)) {
  ...
  for (vector<DirEntry>::const_iterator it = entries.begin();
    ...
    boost::filesystem::path dir_path = datapath;
    dir_path /= it->name();
    ...
    for (vector<DirEntry>::const_iterator it_files = files.begin();
      ...
      if (!ParsePrefixes(path.string(), &prefixes)) {
        return false;
      }
